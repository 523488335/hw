...
from hardware.stereo_camera import two_cam_disparity
...
def buzzer(delay_time=0.2, times=3):
    buzzer_pin = 21
...
def control_algorithm(point_0, point_0_t, point_1, point_1_t, d0, d1):
    ...
    for i in range(36 + 1):
        ...
        point_save.append(point_info)
        points.append(point_format)

    return points, Flag,point_save
...
def main_business():
    while True:
        ...
        # 运动算法处理
        point_edge, Flag_edge ,points_edge= control_algorithm(pointSet_eL, pointSend_e0_t, pointSet_eR, pointSend_e1_t, d0, d1)

        point_mid, Flag_mid,points_mid = control_algorithm(pointSet_mL, pointSend_m0_t, pointSet_mR, pointSend_m1_t, d0, d1)

        normal = [0, 0, 1]
        point_side, Flag_side,points_side = Square_scanning(normal, edge_scan_num, pointSet_edge_scan_l, pointSet_edge_scan_r, d0,
                                                d1)
